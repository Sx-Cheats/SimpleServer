...
extern "C" NTSTATUS   __stdcall NtMakePermanentObject(
     HANDLE Object
);


extern "C" NTSTATUS   __stdcall  NtCreateDirectoryObject(
...
extern "C" NTSTATUS   __stdcall  NtOpenDirectoryObject(
     PHANDLE DirectoryHandle,
     ACCESS_MASK DesiredAccess,
     POBJECT_ATTRIBUTES ObjectAttributes
);
...
extern "C" NTSTATUS    __stdcall NtSuspendProcess(
    HANDLE ProcessHandle
);

extern "C" NTSTATUS    __stdcall NtResumeProcess(
     HANDLE ProcessHandle
);
...
public:
    ...
    [[nodiscard]] static EnumObj<MODULE_ENTRY> * WINAPI GetModules();
    [[nodiscard]] static EnumObj<MODULE_ENTRY> * WINAPI GetImportModules(DEFAULT_SIZE ModuleAddress);
    [[nodiscard]] static EnumObj<MODULE_ENTRY> * WINAPI GetImportModulesEx(std::string ProcessName, DEFAULT_SIZE ModuleAddress);
    [[nodiscard]] static EnumObj<MODULE_ENTRY> * WINAPI GetImportModulesEx(DWORD ProcessId,DEFAULT_SIZE ModuleAddress);
    static PVOID WINAPI ModuleExist(DEFAULT_SIZE base);
    static PVOID WINAPI ModuleExistEx(EnumObj<MODULE_ENTRY>* Modules, std::string ModuleName);

    static std::wstring WINAPI ReadUTF16EX(HANDLE hauth, PWSTR  address,size_t max_size = 256);
    static std::string  WINAPI ReadUTF8EX(HANDLE hauth, PSTR  address, size_t max_size = 256);
    static [[nodiscard]] EnumObj<MODULE_ENTRY>* WINAPI GetModulesEx(const char* AppName);
    static [[nodiscard]] EnumObj<MODULE_ENTRY>* WINAPI GetModulesEx(DWORD pid);
    ...
    static PBYTE AllocEx(std::string ProcessName,size_t size, ACCESS_MASK flProtect = PAGE_READWRITE);
    static PBYTE AllocEx(DWORD Pid, size_t size, ACCESS_MASK flProtect = PAGE_READWRITE);
    ...
    static bool FileExistA(std::string PathFile);
    static bool FileExistW(std::string PathFile);
...
private:
    ...
    static [[nodiscard]] EnumObj<MODULE_ENTRY>* WINAPI __get_modules_ex__(DWORD pid);
